...
def get_distance(predicted_features, index_features, args):
    """
    Computes distance with or without re-ranking
    """
    if args.enable_re_rank:
        distances = re_ranking(probFea=predicted_features.cpu().numpy(), galFea=index_features.cpu().numpy(), k1=args.k1,k2=args.k2, lambda_value=args.lambda_value)
...
def fiq_compute_val_metrics(relative_val_dataset: Dataset, clip_model, index_features: torch.Tensor,
                            index_names: List[str], ref_names_list: List[str], pseudo_tokens: torch.Tensor, args) \
        -> Dict[str, float]:
    """
    ...
    # for k1 in [20]:
    #     for k2 in [5]:
    #         for l in [0.1, 0.2, 0.5, 0.7, 0.9]:
    #             args.k1 = k1
    #             args.k2 = k2
    #             args.lambda_value = l
    #             print("="*20)
    #             print(f"args ::: k1: {args.k1}, k2: {args.k2}, lambda: {args.lambda_value}")
    distances = get_distance(predicted_features, index_features, args)
    ...
    recall_at50 = (torch.sum(labels[:, :50]) / len(labels)).item() * 100
                # print(f"k1 : {args.k1}, k2; {args.k2}, lambda_value: {args.lambda_value}, recall_at10: {recall_at10}, recall_at50: {recall_at50}")
...
def fiq_val_retrieval(dataset_path: str, dress_type: str, image_encoder, text_encoder, ref_names_list: List[str],
                      pseudo_tokens: torch.Tensor, preprocess: callable, args) -> Dict[str, float]:
    """
...
def main():
    ...
    parser.add_argument("--enable_re_rank", action="store_true", help="Whether or not to use re-ranking")
    parser.add_argument("--k1", default=20, type=int, help="re-ranking k1 value")
    parser.add_argument("--k2", default=5, type=int, help="re-ranking k2 value")
    parser.add_argument("--lambda_value", default=0.2, type=float, help="re-ranking lambda value")
